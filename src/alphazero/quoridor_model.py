...
class QuoridorModel(nn.Module):
    ...
    def __init__(self,
        ...
        # Input
        self.conv1 = nn.Conv2d(output_channels=nb_filters,
                               kernel_size=kernel_size,
                               stride=stride)
        self.bn1 = nn.BatchNorm2d
        ...
        self.residual_blocks = [
            ResidualBlock(self.nb_channels, nb_filters, kernel_size, stride)
        ...
        self.policy_dist_size = (self.grid_size - 1) * (
            self.grid_size - 1) * 2 + self.grid_size * self.grid_size
        # TODO: use something else than a flat distribution
        self.policy_conv = nn.Conv2d(nb_filters, 2, kernel_size=1, stride=1, padding="same")
        self.policy_bn = nn.BatchNorm2d
        self.policy_output = nn.Linear(?, self.policy_dist_size)

        # Value head
        self.value_conv = nn.Conv2d(nb_filters, 1, kernel_size=1, stride=1, padding="same")
        self.value_bn = nn.BatchNorm2d
        self.value_fc1 = nn.Linear(, 256)
