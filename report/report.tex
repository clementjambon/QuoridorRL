...
As seen in paragraph \ref{ssec:complexity}, Quoridor is a game with very high state and branching complexities. Like many other games of similar complexity (such as the computer chess, Stockfish\footnote{\url{https://stockfishchess.org/}}, for example), given computational and time constraints for decision making, we cannot perform an exhaustive tree search all the way to a leaf node. Therefore, we turned to an \textit{AlphaGo}-like approach as presented in \cite{alphago}.
...
For the former, if you cannot place anymore walls on the board the agent proceeds by moving in the shortest-path manner which is calculated with the \textit{Djistra} method. 
...
This will result in two things: a \textit{MinMax} search algorithm with moderate runtime, and calculated game state evaluation heuristics that can be used for not only \textit{MinMax} but \textit{MCTS-RAVE} as well.
...
More specifically, in \textit{MCTS} we simulate by self-play $N(s)$ games from state $s$. For a given state $s$ and an action $a$, the value fonction can then be approximated with:
